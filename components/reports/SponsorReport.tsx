...
export default function SponsorReport() {
  ...
  useEffect(() => {
    filterData();
  }, [data, startDate, endDate, sponsorFilter, categoryFilter]);
  ...
  const fetchData = async () => {
    try {
      ...
      const transactionQuery = query(
        transactionsRef,
        where('receivedFromType', '==', 'Sponsor'),
  ...
  const filterData = () => {
    ...
    if (sponsorFilter) {
      filtered = filtered.filter(item => 
        item.receivedFrom?.toLowerCase().includes(sponsorFilter.toLowerCase())
      );
    }
  ...
  const exportToExcel = () => {
    const exportData = filteredData.map(item => ({
      ...
      'Sponsor Name': item.receivedFrom || '-',
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Sponsor Report');

    const fileName = `Sponsor_Finance_Report_${new Date().toISOString().split('T')[0]}.xlsx`;
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Sponsor Finance Report</h1>
        <Button 
